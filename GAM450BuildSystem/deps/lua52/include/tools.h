...
LANES_API struct s_Universe* get_universe(lua_State* L);
...
LANES_API void luaG_dump( lua_State* L );

LANES_API lua_State* luaG_newstate( struct s_Universe* U, lua_State* _from, char const* libs);
LANES_API void luaG_copy_one_time_settings( struct s_Universe* U, lua_State* L, lua_State* L2);
...
LANES_API char const* push_deep_proxy( struct s_Universe* U, lua_State* L, DEEP_PRELUDE* prelude, enum eLookupMode mode_);
LANES_API void free_deep_prelude( lua_State* L, DEEP_PRELUDE* prelude_);

LANES_API int luaG_inter_copy_package(struct s_Universe* U, lua_State* L, lua_State* L2, int package_idx_, enum eLookupMode mode_);

LANES_API int luaG_inter_copy( struct s_Universe* U, lua_State* L, lua_State* L2, uint_t n, enum eLookupMode mode_);
LANES_API int luaG_inter_move( struct s_Universe* U, lua_State* L, lua_State* L2, uint_t n, enum eLookupMode mode_);

LANES_API int luaG_nameof( lua_State* L);
LANES_API int luaG_new_require( lua_State* L);

LANES_API void populate_func_lookup_table( lua_State* L, int _i, char const* _name);
LANES_API void serialize_require( struct s_Universe* U, lua_State *L);
LANES_API void initialize_on_state_create( struct s_Universe* U, lua_State* L);
LANES_API void call_on_state_create( struct s_Universe* U, lua_State* L, lua_State* from_, enum eLookupMode mode_);
