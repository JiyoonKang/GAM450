...
static void expr_discharge(FuncState *fs, ExpDesc *e)
{
  ...
  if (e->k == VUPVAL) {
    ...
    BCReg rc = e->u.s.aux;
    if ((int32_t)rc < 0) {
      if (fs->flags & PROTO_REFERENCE)
      {
        ins = BCINS_ABC(BC_TGETSR, 0, e->u.s.info, ~rc);
      }
      else
...
static void parse_assignment(LexState *ls, LHSVarList *lh, BCReg nvars, BCPos start)
{
  ExpDesc e;
  ...
  if (lex_opt(ls, ',')) {  /* Collect LHS list and recurse upwards. */
    ...
    parse_assignment(ls, &vl, nvars + 1, start);
  }
  else {  /* Parse RHS. */
    BCReg nexps;
    ...
    if (ls->token == '=')
      lj_lex_next(ls);
    else if (ls->token == '+' || ls->token == '-' || ls->token == '*' || ls->token == '/')
    {
      //make sure is is <token>=
      if (ls->current == '=')
      {
        //Figure out what instruction to emit
        BinOpr op = token2binop(ls->token);

        //Save lhs variable in the desc, and reserve a register that it will *eventually* be put into
        e = lh->v;
        ...
        //Do the binop on the rhs
        ExpDesc v;
        BinOpr nextop = expr_binop(ls, &v, 0);
        
        //If it needs to, keep evaluating the rhs
        while(nextop != OPR_NOBINOPR)
        {
          ExpDesc v2;
          BinOpr tempop;
          bcemit_binop_left(ls->fs, nextop, &v);
          //Parse binary expression with higher priority.
          tempop = expr_binop(ls, &v2, priority[nextop].right);
          bcemit_binop(ls->fs, nextop, &v, &v2);
          nextop = tempop;
        } 

        //When the entire rhs has been evaluated, stick our operation in last
        bcemit_binop_left(ls->fs, op, &e);
        bcemit_binop(ls->fs, op, &e, &v);

        //Finally, store the lhs
...
static void parse_call_assign(LexState *ls)
{
  ...
  //Save the location of the starting statement (pc is 1 ahead)
  BCPos pc = ls->fs->pc - 1;
  expr_primary(ls, &vl.v);
