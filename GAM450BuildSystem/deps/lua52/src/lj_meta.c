...
cTValue *lj_meta_tget(lua_State *L, cTValue *o, cTValue *k, int isRef)
{
  int loop;
...
TValue *lj_meta_equal(lua_State *L, GCobj *o1, void *o2, int ne, int cond)
{ 
  /*
  Conditions:
  0 - gcValue
  ...
  */
  ...
  if (mo) {
    ...
    {
      GCobj *p2 = *(GCobj **)o2;
      unsigned mask = (unsigned)*(void **)((char *)o2 + 4);
      uint32_t it2 = mask;
      setgcV(L, top + 3, p2, it2);
    }
      break;
    //Strings
    case 1:
    {
      GCobj *p2 = *(GCobj **)o2;
      uint32_t it2 = ~(uint32_t)p2->gch.gct;
      setgcV(L, top + 3, p2, it2);
    }
      break;
    //Numbers
    ...
    case 3:
    {
      unsigned mask = (unsigned) o2;
      if (mask == LJ_TNIL)
      {
        setnilV(top + 3);
      }
      else if (mask == LJ_TTRUE)
      {
        setboolV(top + 3, 1);
      }
      else if (mask == LJ_TFALSE)
      {
        setboolV(top + 3, 0);
      }
      else
        copyTV(L, top + 3, (L->base + 7));
    }
      break;
    default:
      break;
    ...
    //if (p2 == NULL)
    //{
    //  copyTV(L, top + 3, (L->top - 3));
    //}
    //else if ((unsigned)mask == LJ_TNIL)
    //{
    //  setnilV(top + 3);
    //}
    //else if ((unsigned)mask == LJ_TTRUE)
    //{
    //  setboolV(top + 3, 1);
    //}
    //else if ((unsigned)mask == LJ_TFALSE)
    //{
    //  setboolV(top + 3, 0);
    //}
    //else if (~(unsigned int)mask < 13)
    //{
    //  copyTV(L, top + 3, (L->top - 3));
    //}
    //else if (((unsigned int)mask & 0x3ff00000))
    //{
    //  copyTV(L, top + 3, (L->top - 3));
    //}
    //else
    //{
      //copyTV(L, top + 3, oV);
      //uint32_t it2 = ~(uint32_t)o2->gch.gct;
      //setgcV(L, top + 3, o2, it2);
    //}
...
LJ_FUNCA TValue *lj_meta_equal_n(lua_State *L, GCobj *o1, cTValue *o2, int ne)
{
  /* Field metatable must be at same offset for GCtab and GCudata! */
  cTValue *mo = lj_meta_fast(L, tabref(o1->gch.metatable), MM_eq);
