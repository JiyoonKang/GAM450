...
GCobj *lua_movegcdata(lua_State *L, GCobj *o)
{
  /*
  (o)->gch.gct == ~LJ_TSTR   , &(o)->str)
  (o)->gch.gct == ~LJ_TUPVAL , &(o)->uv)
  (o)->gch.gct == ~LJ_TTHREAD, &(o)->th)
  (o)->gch.gct == ~LJ_TPROTO , &(o)->pt)
  (o)->gch.gct == ~LJ_TFUNC  , &(o)->fn)
  (o)->gch.gct == ~LJ_TCDATA , &(o)->cd)
  (o)->gch.gct == ~LJ_TTAB   , &(o)->tab)
  (o)->gch.gct == ~LJ_TUDATA , &(o)->ud)
  ...
  {
    GCstr *str = &(o)->str;
    return (GCobj *)lj_str_new(L, (const char *)(str + sizeof(GCstr)), 0);
  }
  else if ((o)->gch.gct == ~LJ_TFUNC)
  {
    GCfunc *func = &(o)->fn;
    lua_movegcdata(L, gcref(func->l.env));
  }
  else if ((o)->gch.gct == ~LJ_TTAB)
  {
    GCtab *table = &(o)->tab;
    GCtab *new_table = lj_tab_dup(L, table);
...
LUA_API void lua_move(lua_State *from, lua_State *to, int n)
{
  /*
  TValue *f, *t;
  if (from == to) return;
  ...
  while (--n >= 0)
  {
    ...
    //Move the gcdata to the other state, push it onto the stack
    GCobj *obj = lua_movegcdata(to, gcref(f->gcr));
    setgcV(to, t, obj, itype(f));
  }
  from->top = f;
  */
...
LUA_API int lua_equal(lua_State *L, int idx1, int idx2)
{
  ...
  if (tvisint(o1) && tvisint(o2)) {
    ...
    TValue *base = lj_meta_equal(L, gcV(o1), (void *)gcV(o2), 0, 0);
    if ((uintptr_t)base <= 1) {
