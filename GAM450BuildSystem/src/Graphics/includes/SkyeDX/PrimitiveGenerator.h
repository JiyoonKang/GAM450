...
namespace Skye
{
	...
	private:
		...
		void BuildCylinderTopCap(float bottomRadius, float topRadius, float height, unsigned sliceCount, unsigned stackCount, ModelData& ModelData, float4 color);
		void BuildCylinderBottomCap(float bottomRadius, float topRadius, float height, unsigned sliceCount, unsigned stackCount, ModelData& ModelData, float4 color);
		static void __fastcall TessellatePatch(ModelData& ModelData, TeapotPatch const& patch, size_t tessellation, DirectX::FXMVECTOR scale, bool isMirrored, float4 color);
	};
